...
export default function Home() {
  ...
  return <main>
    ...
    <section className="page shell">
      <div className="intro"><p className="step">快速、安全、簡單</p><h1>選擇儲值方案</h1><p>1 元台幣可兌換 48 點，選擇方案後再選擇付款方式。</p></div>

      <section className="panel" id="packages" aria-labelledby="package-title">
        <div className="panelHead"><div><span className="number">1</span><div><h2 id="package-title">選擇儲值方案</h2><p>兌換比例：TWD 1＝48 點</p></div></div><span className="secure">● 安全加密付款</span></div>
      ...
      <section className="checkout" aria-labelledby="checkout-title">
        <div><span className="number">2</span><div><h2 id="checkout-title">選擇付款方式</h2><p>選擇信用卡或超商，最後只按一次「前往付款」。</p></div></div>

        <div className="methodTabs" role="tablist" aria-label="付款方式">
          {(Object.keys(methods) as Method[]).map(key => <button key={key} type="button" role="tab" aria-selected={method === key} className={method === key ? 'active' : ''} onClick={() => changeMethod(key)}>
            <span>{key === 'card' ? '▰' : '店'}</span><div><strong>{methods[key].label}</strong><small>{methods[key].note}</small></div>
          </button>)}
        </div>

        <div className="summary"><span>已選擇 {currentPoints} 點</span><strong>TWD {currentPrice.toLocaleString()}</strong></div>
        {paymentUrl ? <a className="checkoutButton" href={paymentUrl} target="_blank" rel="noopener noreferrer">前往付款 <span>→</span></a> : <button className="checkoutButton unavailable" type="button" disabled>此付款方式暫不支援此金額</button>}
      </section>

      <section className="help"><h2>付款安心有保障</h2><p>目前提供兩種付款方式：信用卡、超商。選擇付款方式後，只需要按一次「前往付款」即可進入對應付款頁。</p></section>
    </section>
